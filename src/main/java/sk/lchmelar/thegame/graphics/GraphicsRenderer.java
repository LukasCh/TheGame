...
public class GraphicsRenderer extends Thread {
	...
	public void run() {
		...
		if (TheGame.debugMode)
			debugLastRenderDelta = lastRender;
		main: while (isRunning) {
			...
			do {
				...
				if (TheGame.debugMode)
					debugLastRenderDelta = delta;
				lastRender = System.currentTimeMillis();
				renderGame(backgroundGraphics, viewport);

				/*
				 * if (scale != 1) { bg.drawImage(background,
				 * cameraX.intValue(), cameraY.intValue(), width * scale, height
				 * * scale, 0, 0, width, height, null); } else {
				 */
